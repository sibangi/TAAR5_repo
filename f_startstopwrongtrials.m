...
function [Behav] = f_startstopwrongtrials(Datas)
    ...
    Wrong = zeros(length(TrialOnset),1);
    for tr = 1:length(TrialOnset)
        ...
        % check if that trial is Wronged or not
        clear ind_rew
        ind_rew = find((Data(TrialOnset(tr):TrialOffset(tr),2)==34) | (Data(TrialOnset(tr):TrialOffset(tr),2)==29) | ...
...
%         end
    ...
    Behav = [HourStartAll HourEndAll Wrong];
